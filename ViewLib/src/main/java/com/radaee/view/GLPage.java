...
public class GLPage implements ILayoutView.IVPage{
    ...
    public final void gl_draw(GL10 gl10, GLThread thread, int def_text, int orgx, int orgy, int w, int h)
    {
        ...
        {
            boolean all_ok = true;
            int cleft = -left - GLBlock.m_cell_size;
            int ctop = -top - GLBlock.m_cell_size;
            int cright = w - left + GLBlock.m_cell_size;
            int cbottom = h - top + GLBlock.m_cell_size;
            ...
            if(m_blks_zoom == null)
            {
                for(int ib = 0; ib < m_blks.length; ib++)
                {
                    ...
                    glb.gl_draw(gl10, def_text, cleft, ctop, cright, cbottom);
                }
            }
        }
        ...
        for(int ibb = 0; ibb < m_blks_zoom.length; ibb++)
        {
            ...
            glb.gl_draw(gl10, def_text, zleft, ztop, zright, zbottom);
        }
    }
