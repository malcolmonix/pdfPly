...
public class PDFLayoutDual extends PDFLayout
{
        ...
        if( m_h > m_w )//vertical
        {
            ...
            while( ccur < ccnt )
            {
                ...
                if( m_vert_dual != null && ccur < m_vert_dual.length && m_vert_dual[ccur] && pcur < pcnt - 1 )
                {
                    ...
                    if(m_page_align_top)
                    {
                        m_pages[pcur].vLayout(left + (cw - w) / 2, m_page_gap / 2, m_scale, clip);
                        m_pages[pcur + 1].vLayout(m_pages[pcur].GetX() + m_pages[pcur].GetWidth(), m_page_gap / 2, m_scale, clip);
                    }
                    else
                    {
                        m_pages[pcur].vLayout(left + (cw - w) / 2, (int) (m_th - m_doc.GetPageHeight(pcur) * m_scale) / 2, m_scale, clip);
                        m_pages[pcur + 1].vLayout(m_pages[pcur].GetX() + m_pages[pcur].GetWidth(), (int) (m_th - m_doc.GetPageHeight(pcur + 1) * m_scale) / 2, m_scale, clip);
                    }
                    pcur += 2;
                }
                else
        ...
        else
        {
            while( pcur < pcnt )
            {
                ...
                if( (m_horz_dual == null || ccnt >= m_horz_dual.length || m_horz_dual[ccnt]) && pcur < pcnt - 1 )
                {
                    ...
                    if( max_h < h ) max_h = h;
                    if(Global.fit_different_page_size && m_scales[pcur] == 0) {
                        m_scales[pcur] = ((float)(m_h - m_page_gap)) / m_doc.GetPageHeight(pcur);
                        m_scales_min[pcur] = ((float)(m_h - m_page_gap)) / m_doc.GetPageHeight(pcur);
                        m_scales[pcur + 1] = ((float)(m_h - m_page_gap)) / m_doc.GetPageHeight(pcur + 1);
                        m_scales_min[pcur + 1] = ((float)(m_h - m_page_gap)) / m_doc.GetPageHeight(pcur + 1);
                    }
                    pcur += 2;
                }
                else
    ...
    public void vGotoPage( int pageno )
    {
        ...
        if(Global.fit_different_page_size && vGetScale() > vGetMinScale())
            vZoomSet(vGetWidth()/2, vGetHeight() / 2, vGetPos(0,0), 1);
        vScrollAbort();
        int ccur = 0;
        while( ccur < m_cells.length )
        {
    ...
    public void vZoomSet(int vx, int vy, PDFPos pos, float zoom)
    {
        ...
        if(Global.fit_different_page_size) {
            float newScale = zoom * m_scales_min[pos.pageno];
            if(newScale < m_scales_min[pos.pageno]) newScale = m_scales_min[pos.pageno];
            if(newScale > vGetMaxScale()) newScale = vGetMaxScale();
            m_scales[pos.pageno] = newScale;
            if(Global.def_view == 6 && m_w > m_h) {
                if (m_cells[(pos.pageno + 1) / 2].page_right == pos.pageno + 1) {
                    newScale = zoom * m_scales_min[pos.pageno + 1];
                    if (newScale < m_scales_min[pos.pageno + 1])
                        newScale = m_scales_min[pos.pageno + 1];
                    if (newScale > vGetMaxScale()) newScale = vGetMaxScale();
                    m_scales[pos.pageno + 1] = newScale;
                } else if (m_cells[(pos.pageno + 1) / 2].page_left == pos.pageno - 1) {
                    newScale = zoom * m_scales_min[pos.pageno - 1];
                    if (newScale < m_scales_min[pos.pageno - 1])
                        newScale = m_scales_min[pos.pageno - 1];
                    if (newScale > vGetMaxScale()) newScale = vGetMaxScale();
