...
public class GLBlock {
    ...
    public static void drawQuadColor(GL10 gl10, int vx00, int vy00, int vx10, int vy10, int vx01, int vy01, int vx11, int vy11, float r, float g, float b)
    {
    ...
    private void drawLT(GL10 gl10, int texture, int x, int y, int text_shadow)
    {
        gl10.glBindTexture(GL10.GL_TEXTURE_2D, texture);//bind texture
        float sr = 0.5f;
        float sg = 0.5f;
        ...
        if(a < -99999 || p11y > 30000 + m_w)//vertical
        {
            ...
            drawQuadColor(gl10, x << 16, 0, right << 16, 0,
                    x << 16, m_h << 16, right << 16, m_h << 16, er, eg, eb);
            int shadow = remain / 3;
            for(int pos = 0; pos < 32; pos++)
            {
                drawQuadColor(gl10, (right + shadow * pos / 32) << 16, 0, (right + shadow * (pos + 1) / 32) << 16, 0,
        ...
        else if(x == m_w || p22x < -30000)//horizon
        {
            ...
            drawQuadFixed(gl10, 0, (bot + (y >> 3)) << 16, m_w << 16, (bot + (y >> 3)) << 16,
                    ...
                    0, 0, 1 << 16, 0, 0, 1 << 16, 1 << 16, 1 << 16);
            gl10.glBindTexture(GL10.GL_TEXTURE_2D, 0);//bind texture
            drawQuadColor(gl10, 0, y << 16, m_w << 16, y << 16,
                    0, bot << 16, m_w << 16, bot << 16, er, eg, eb);
            int shadow = remain / 3;
            for(int pos = 0; pos < 32; pos++)
            {
                drawQuadColor(gl10, 0, (bot + shadow * pos / 32) << 16, m_w << 16, (bot + shadow * pos / 32) << 16,
        ...
        else
        {
            ...
            for (int pos = 0; pos < 64; pos += 2)//画四边形条目
            {
                ...
                drawQuadColor(gl10, b11[pos] << 16, b11[pos + 1] << 16, b11[pos + 2] << 16, b11[pos + 3] << 16,
    ...
    private void drawRB(GL10 gl10, int texture, int x, int y, int text_shadow)
    {
        gl10.glBindTexture(GL10.GL_TEXTURE_2D, texture);//bind texture
        float sr = 0.5f;
        float sg = 0.5f;
    ...
    protected final void gl_draw_curl(GL10 gl10, int def_text, int type, int x, int y, int text_shadow)
    {
        ...
        {
            case 1:
                if(m_texture == 0) drawLT(gl10, def_text, x, y, text_shadow);
                else drawLT(gl10, m_texture, x, y, text_shadow);
                break;
            case 2:
                if(m_texture == 0) drawRB(gl10, def_text, x, y, text_shadow);
                else drawRB(gl10, m_texture, x, y, text_shadow);
                break;
            default:
